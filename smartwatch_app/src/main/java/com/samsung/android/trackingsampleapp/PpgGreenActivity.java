...
import com.samsung.android.trackingsampleapp.databinding.ActivityPpgGreenBinding;
...
public class PpgGreenActivity extends FragmentActivity {

	private final String TAG = PpgGreenActivity.class.getSimpleName();
	private ActivityPpgGreenBinding binding;
	...
	private HealthTracker ppgGreenTracker = null;
	...
	private final ConnectionListener connectionListener = new ConnectionListener() {
		...
		public void onConnectionSuccess() {
			...
			binding.ppgStartButton.setOnClickListener(view -> {
			...
			binding.ppgStopButton.setOnClickListener(view -> {
				Log.i(TAG, " unsetEventListener called ");
				if (ppgGreenTracker != null) {
			...
			binding.ppgFlushButton.setOnClickListener(view -> {
				Log.i(TAG, " flush called ");
				if(!ppgGreenTracker.flush()) {
		...
		public void onConnectionFailed(HealthTrackerException e) {
			if(e.hasResolution()) {
				e.resolve(PpgGreenActivity.this);
			}
		...
		public void onDataReceived(@NonNull List<DataPoint> list) {
			if (list.size() != 0) {
				...
				for(DataPoint dataPoint : list) {
					...
					Log.i(TAG, "Ppg Green Value : " + dataPoint.getValue(ValueKey.PpgGreenSet.PPG_GREEN));
					sum += dataPoint.getValue(ValueKey.PpgGreenSet.PPG_GREEN);
				}
				int avgPpgValue = sum/list.size();
				runOnUiThread(() -> binding.greenRawValue.setText(String.valueOf(avgPpgValue)));
			} else {
	...
	public final void setUp() {
		binding = DataBindingUtil.setContentView(this, R.layout.activity_ppg_green);
