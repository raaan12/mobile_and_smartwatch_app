...
public class PpgRedActivity extends FragmentActivity {

	private final String TAG = PpgRedActivity.class.getSimpleName();
	private ActivityPpgRedBinding binding;
	...
	private HealthTracker ppgRedTracker = null;
	...
	private final ConnectionListener connectionListener = new ConnectionListener() {
		...
		public void onConnectionSuccess() {
			...
			try {
				ppgRedTracker = healthTrackingService.getHealthTracker(HealthTrackerType.PPG_RED);
			...
			binding.ppgRedStartButton.setOnClickListener(view -> {
				Log.i(TAG, " setEventListener called ");
				if(!isHandlerRunning) {
					handler.post(() -> {
						ppgRedTracker.setEventListener(trackerEventListener);
						isHandlerRunning = true;
					});
				}
			});
			binding.ppgRedStopButton.setOnClickListener(view -> {
				Log.i(TAG, " unsetEventListener called ");
				if (ppgRedTracker != null) {
		...
		public void onDataReceived(@NonNull List<DataPoint> list) {
			if (list.size() != 0) {
				...
				for(DataPoint dataPoint : list) {
					...
					Log.i(TAG, "Ppg Red Value : " + dataPoint.getValue(ValueKey.PpgRedSet.PPG_RED));
					runOnUiThread(() -> binding.redRawValue.setText(String.valueOf(dataPoint.getValue(ValueKey.PpgRedSet.PPG_RED))));
				}
			} else {
	...
	public final void setUp() {
		binding = DataBindingUtil.setContentView(this, R.layout.activity_ppg_red);
